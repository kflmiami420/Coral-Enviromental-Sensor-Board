#  This script is modified from the original to convert  C to F temp 
#  This script is modified from the original to convert  kPa to mBar 
...
def main():
    ...
    enviro = EnviroBoard()
#    enviro.pressure = enviro.pressure * 10.0
#    enviro.temperature = enviro.temperature * (9.0 / 5.0) + 32
    with CloudIot(args.cloud_config) as cloud:
        ...
        for read_count in itertools.count():
            ...
            msg += 'Pressure:%.2f mbar' % _none_to_nan(sensors['pressure'])
            update_display(enviro.display, msg)
            sleep(args.display_duration)
